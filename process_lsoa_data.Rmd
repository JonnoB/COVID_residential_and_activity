...
tsne_mat <-covid_msoa_time %>%
select(areaCode, date, rolling_sum) %>%
  pivot_wider(., values_from = rolling_sum, names_from = date) %>%
  select(-areaCode) %>%
  Rtsne()

row_fract<- rowSums(is.na(tsne_mat))/ncol(tsne_mat) 
col_fract<- colSums(is.na(tsne_mat))/nrow(tsne_mat) 
...
https://api.coronavirus.data.gov.uk/v2/data?areaType=msoa&areaCode=E09000003&metric=newCasesBySpecimenDateRollingRate&format=csv
...
msoa_class <- geog_lookup %>%
  group_by(SOAC11CD, SOAC11NM, MSOA11CD) %>%
  summarise(counts = n()) %>%
  group_by(MSOA11CD) %>%
